...
A working demo of [`REPLAYABLE_STATE_PROPOSAL.md`](../REPLAYABLE_STATE_PROPOSAL.md): take collected
episodes and show that almost everything interesting about them is a **deterministic offline
projection of the logged state trace** — new views, depth/segmentation, 3D boxes, point clouds,
spatial-relation scene graphs, robot kinematics, success labels — none of which needs the simulator
or re-collection.

The data this consumes is exactly the per-frame actor-pose trace that
[`robo_negative`](../robo_negative/README.md) logs (`targeted_state`), plus the standard bench
`joint_action` / `endpose` / camera streams.
...
- **Multi-scene:** switch between 5 episodes (clean baseline / shift_object / shift_target,
  d6 baseline / shift_obstacle) and **compare twins** side-by-side (synced, with a Δ divergence readout)
...
- The d6 scene has ~14 objects — use the Objects **none** toggle or turn off "Relations on view" to declutter.
