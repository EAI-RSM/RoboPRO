...
class put_sauce_can_in_cabinet(Kitchen_base_large):
    SAUCE_CAN_MASS = 0.1
    SAUCE_CAN_SPAWN_Z_OFFSET = 0.01
    ...
    # Placement anchor in cabinet base-link local frame.
    CABINET_PLACE_LOCAL = np.array([0.08, -0.10, -0.12], dtype=float)

    APPROACH_DELTA_1 = dict(x=-0.05, y=0.14, z=0.37)
    ...
    @staticmethod
    def _right_side_contact_points(y_center: float) -> list:
        # Keep the same right-side grasp style as put_can_in_cabinet.
        return [
    ...
    def _ensure_sauce_can_grasp_metadata(self) -> None:
        ...
        cfg["contact_points_pose"] = self._right_side_contact_points(y_center)
