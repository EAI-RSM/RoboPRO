...
class place_bowl_in_dishrack_ks(KitchenS_base_task):

    # Bowl has IK-reachable side contact points, so we use the standard
    ...
    def play_once(self):
        ...
        # Rack is in curobo collision — pop it so the planner isn't blocked
        # by bowl-rack overlap checks at the target.
        self.collision_list = [
            e for e in self.collision_list if e.get("actor") is not self.dishrack
        ]
        self.update_world()

        # Drop above rack basin. Two-stage move (hover then drop) mirrors
        # the working plate recipe — gives the planner a reachable midpoint
        # seed before the final rack target. Uses the home EE quat
        # ([0.707, 0, 0, 0.707], gripper facing front) since the identity
        # quat is not IK-reachable from the side-grasp config.
