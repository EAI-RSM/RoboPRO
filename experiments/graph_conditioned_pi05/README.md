...
The `graph_delta_homogeneous_action_stages_v1` treatment uses the same two-field prompt schema in every supported phase: `Task objective: <original task>` followed by `Current stage: <atomic instruction>`. Grasp uses `Pick up <target>.`; transport uses `Keep holding the object. Move it <direction> into/onto <destination>.`; final alignment uses `Place the held object into/onto <destination>.`; and release uses `Release the held object in/on <destination>.` The relation-specific prepositions are derived from the task goal, so containment and support tasks remain semantically distinct. It sends no `Nodes:` or `Relations:` section, aliases, sizes, end-effector coordinates, or retrieved relation prose.
