...
Job `11792` (`graph_delta_compact_phase_guidance_v1`, 4/10 on the frozen sauce-can seeds) is the frozen representation-ablation baseline. Legacy job `10581` (6/10 on the same seeds) is retained as the performance target to reach and exceed.

The `graph_delta_compact_collision_replan_v1` treatment changes that baseline only when a new valid robot-to-nontarget-object collision begins during grasp or placement. The event interrupts the unused action chunk immediately and adds `Avoid the <object> and continue the task.` to exactly the next replanned prompt. Persistent contact does not repeatedly interrupt chunks, and the prompt then returns to the compact baseline. It still sends no general `Nodes:` or `Relations:` section, aliases, sizes, end-effector coordinates, or unrelated relation prose.
