...
The `graph_delta_compact_phase_guidance_v1` baseline sends the original task instruction during grasp and appends exactly one compact action sentence during placement or release. It sends no `Nodes:` or `Relations:` section, aliases, sizes, end-effector coordinates, or retrieved relation prose. Episode JSONL records store this treatment version separately from legacy runs and retrieved-text treatments so staged prompt ablations remain identifiable.
