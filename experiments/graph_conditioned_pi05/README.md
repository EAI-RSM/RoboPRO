...
The `graph_delta_pregrasp_substages_v1` treatment selects an approach arm only when exactly one validated straight target corridor is clear and every alternative corridor is blocked. Approach says `Use the <left/right> gripper to approach <target>.` or, without unambiguous arm evidence, `Move the gripper toward <target>.` Near the target it says `Align the <arm> gripper with <target>.`; on contact it says `Close the <arm> gripper to grasp <target>.` A collision warning is added only when the nearest validated blocker's AABB is within 20 cm of the blocked end-effector. Far from the target, nearest-arm geometry never overrides the collision-aware clear-arm choice. Placement and release prompts, transitions, and gripper protection remain unchanged.
