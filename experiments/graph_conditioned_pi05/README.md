...
The `graph_delta_actionable_imminent_collision_v1` treatment selects an arm only when exactly one validated straight target corridor is clear and every alternative corridor is blocked. Its normal cue is `Use the <left/right> gripper to approach <target> and pick it up.` A collision warning is added only when the nearest validated blocker's AABB is within 20 cm of the blocked end-effector position: `Collision risk: <obstacle> blocks the <blocked-side> gripper. Use the <clear-side> gripper ...` Thus the obstacle is attributed to the corridor it actually blocks, the alternative action is explicit, and distant blockers do not dominate the prompt. Missing or ambiguous evidence falls back to `Pick up <target>.` Transport, placement, release, controller thresholds, and replanning behavior are unchanged.
