...
The `graph_delta_approach_side_availability_v1` treatment retains the homogeneous two-field prompt schema. During grasp, if exactly one gripper has a validated clear straight corridor to the target while the other is blocked, the current stage becomes `Approach <target> with the <left/right> gripper and pick it up.` Otherwise it conservatively remains `Pick up <target>.` This is gripper-side availability, not a claim about an arbitrary target-relative approach direction. Transport, final alignment, and release wording are unchanged from `graph_delta_homogeneous_action_stages_v1`. No serialized nodes, aliases, geometry, or general relation prose are sent.
