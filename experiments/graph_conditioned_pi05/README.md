...
The `graph_delta_postgrasp_retrieved_text_v1` treatment sends dependency-safe `Nodes:` and `Relations:` sections only during placement and release. Deterministic aliases (`T1...`, `D1...`, `O1...`, `L`, and `R`) declare each selected entity once. Target and destination declarations are mandatory; other nodes are selected only with a relation that references them. The server uses the checkpoint tokenizer and the shared priority-maximizing packer, dropping lower-priority relations first when the graph budget or full prompt limit is reached. Episode JSONL records store this treatment version separately from both compact-guidance-only runs and the earlier treatment that also inserted retrieved text during grasping.
