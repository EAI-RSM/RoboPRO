...
The `graph_delta_transport_language_v1` treatment sends the original task instruction during grasp. While the held target still needs horizontal transport, it appends `Keep holding the object. Move it <direction> into/onto <destination>.` Once horizontally aligned, it retains the compact `Place the held object into/onto <destination>.` instruction; release retains `Release the held object in/on <destination>.` It sends no `Nodes:` or `Relations:` section, aliases, sizes, end-effector coordinates, or retrieved relation prose. This changes only transport-stage wording relative to `graph_delta_compact_phase_guidance_v1`; the controller, thresholds, and other phase prompts are unchanged.
