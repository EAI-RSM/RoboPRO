...
The `graph_delta_obstacle_relative_to_arm_v1` treatment retains approach-side availability and adds one coarse obstacle statement when its geometry is known. It selects the nearest object blocking the rejected gripper's target corridor and describes that obstacle relative to the selected clear gripper in the robot-base frame, for example `The kettle is forward and left of the right gripper.` Components below 5 cm are omitted. If obstacle, end-effector, or robot-base geometry is unavailable, the prompt retains only `Approach <target> with the <left/right> gripper and pick it up.` If no uniquely clear gripper exists, it remains `Pick up <target>.` Transport, placement, release, controller thresholds, and replanning behavior are unchanged.
