...
Port status — **code complete, never successfully inferred.** Done and verified: `.venv` built via
...
**The smoke test is BLOCKED by the expert feasibility gate, not by the port.** Env boots, SAPIEN
inits, the jax server binds and the client handshakes — but inference is never reached, so model
load+infer remains UNVERIFIED. The crash is the two-pass expert gate ([[repo_task_assets]]):
...
**`GPU_SPEC` MUST be `0:0`** (single 16 GB card). To validate the policy end to end, run on a
task/scene where the expert IS feasible, or add a path that skips `play_once`.
