...
# When this file is the entry point, collect/_env fills BENCH_ROOT / DATA_ROOT.
if "BENCH_ROOT" not in os.environ:
    _collect = str(Path(__file__).resolve().parent)
...
def _resolve_save_path(save_path: str) -> Path:
    """Map a task-config save_path (e.g. `./data`) onto DATA_ROOT."""
...
def replace_placeholders(instruction: str, episode_params: Dict[str, str]) -> str:
    ...
    for key, value in stripped_episode_params.items():
        ...
        json_path = os.path.join(str(OBJECTS_DESCRIPTION_DIR), value + ".json")
...
def load_task_instructions(task_name: str) -> Dict[str, Any]:
    ...
    if bench_instruction_path.exists():
        with open(str(bench_instruction_path), "r") as f:
            return json.load(f)

    # Bench tasks without a dedicated templated json fall back to the
    # one-line entry in plain_instructions.json (scene-keyed).
    plain_path = bench_root / "bench_description" / "plain_instructions.json"
    if plain_path.exists():
        with open(str(plain_path), "r") as f:
            plain = json.load(f)
        for scene_tasks in plain.values():
            if task_name in scene_tasks:
                instr = scene_tasks[task_name]
                return {"seen": [instr], "unseen": [instr]}

    raise FileNotFoundError(
        f"No instructions for task '{task_name}' in "
        f"bench_description/task_instructions or plain_instructions.json"
    )
...
if __name__ == "__main__":
    ...
    setting_file = bench_root / "bench_task_config" / f"{args.setting}.yml"
    if not setting_file.exists():
        raise SystemExit(f"No task config: {setting_file}")
