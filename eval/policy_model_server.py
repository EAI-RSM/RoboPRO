        ...
        with client_socket:
            while self.running:
                try:
                    ...
                    # Call method with or without obs. ModelClient may pack
                    # multi-arg invocations as `{"_args": [...]}`; unpack.
                    if isinstance(obs, dict) and "_args" in obs:
                        result = method(*obs["_args"])
...
def get_camera_config(camera_type):
    ...
    cfg_path = os.path.join(os.environ["SIM_ROOT"], "task_config", "_camera_config.yml")
