...
See script/verify_replay_determinism.py for the harness that measures all of this.
"""
import importlib
import os
...
def build_args(task_name, task_config):
    ...
    if os.getenv("ROBOTWIN_BENCH_TASK") == "bench":
        cfg_path = f"{root}/bench_task_config/{task_config}.yml"
    else:
...
def make_task(task_name):
    """Instantiate a task by name, searching the bench env packages."""
    for mod in (f"bench_envs.{task_name}",
                f"bench_envs.office.{task_name}",
                f"bench_envs.study.{task_name}",
                f"bench_envs.kitchenl.{task_name}",
                f"bench_envs.kitchens.{task_name}",
                f"envs.{task_name}"):
        try:
            return getattr(importlib.import_module(mod), task_name)()
        except ModuleNotFoundError:
            continue
    raise SystemExit(f"task {task_name} not found in any env package")
