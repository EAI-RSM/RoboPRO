"""
VENDORED (do not edit as active tooling): frozen snapshot of Hamid's expert from
`analyze_occluder_visibility.py` on branch 35-occluder-spawn (commit 7f61a7c). Imported
by the testbench's `--plan-algo hamid` to run Hamid's single deployable planner as a
controlled comparison point. Only `make_occluder_task` is used from here; run()/plotting
are dormant (guarded under __main__).

Scene-paired splice vs the raw snapshot (so `hamid` faces the SAME occluder scenes as the
testbench's other planners): TARGET_YLIM narrowed to the testbench band, and load_actors
gained the occluder-reset + clutter prohibit-area. Search "SCENE-PAIR" for the three edits.
To refresh Hamid's algorithm: re-copy from 35-occluder-spawn and re-apply the SCENE-PAIR edits.
...
TARGET_YLIM = (0.1, 0.15)   # SCENE-PAIR: testbench band (was (0.0, 0.20)) so hamid faces the same bottle distribution
...
def make_occluder_task():
    ...
    class OccluderTask(Base):
        ...
        def load_actors(self):
            # SCENE-PAIR: env is REUSED across builds (clean / no-occluder / occluder), so clear any
            # occluder from a previous setup_demo up front -- otherwise a stale toppled box leaks
            # into the next build's stability gate. (matches testbench load_actors)
            self.occluder = None
            ...
            if self.spawn_occluder:
                ...
                # SCENE-PAIR: fence procedural table clutter off the occluder so it can't land
                # against/on the box and topple it during settle. (matches testbench load_actors)
