...
from lib.vla_reporting import metric_scene_manifest
...
from validate_task_geometric_ranking import summarize_task_calibration
...
def test_metric_commit_repair_and_provisional_reporting():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        _sync_metric_jsonl(metric_dir, repaired)
...
def test_stock_calibration_summary_has_no_arbitrary_gate():
    records = []
    for seed, gated, geom in ((1, 0.05, 0.06), (2, 0.08, 0.09), (3, 0.12, 0.14)):
        records.append(
            {
                "status": "ok",
                "scene_id": f"scene-{seed}",
                "seed": seed,
                "eps_gated_min_rank": gated,
                "eps_gated_min_rank_unbounded": False,
                "eps_geom_min_rank": geom,
                "eps_geom_min_rank_unbounded": False,
                "legs": [
                    {
                        "index": 0,
                        "kind": "pre_grasp",
                        "merged_gated": True,
                        "merged_geom": True,
                        "eps_gated_rank": gated,
                        "eps_gated_rank_unbounded": False,
                        "eps_geom_rank": geom,
                        "eps_geom_rank_unbounded": False,
                    }
                ],
            }
        )
    summary = summarize_task_calibration(records)
    assert summary["scene_min"]["spearman"] == 1.0
    assert summary["no_arbitrary_minimum_n_gate"] is True
    assert summary["decision"] == "USER_REVIEW_REQUIRED"
...
def main():
    ...
    test_stock_calibration_summary_has_no_arbitrary_gate()
