...
from envs._GLOBAL_CONFIGS import GRASP_DIRECTION_DIC  # used by --plan-algo reachability
...
# ---- Reachability planner constants (used only by --plan-algo reachability) ----
...
def make_occluder_task():
    ...
    class OccluderTask(Base):
        ...
        #   "reachability" -> _play_once_reachability (the check_ik_batch candidate-search
        ...
        def _play_once_reachability(self):
            # Expert plan. FORWARD (grasp) below is fixed. BACKWARD (placement) is a list
            # of subgoals returned by self._backward_subgoal_poses_reach() -- EDIT THAT METHOD to
        ...
        def _backward_subgoal_poses_reach(self, arm_tag, x_side, clearance_z, lift_pose):
        ...
        def _around_box_waypoint(self, arm_tag, ref_pose, gap=SIDE_WAYPOINT_GAPS[1], z_lift=0.0,
            ...
            if not (self.spawn_occluder and getattr(self, "occluder", None) is not None):
                return None
            wp = list(ref_pose)
            wp[0] = self._box_side_x_reach(arm_tag, gap=gap)
        ...
        def _rank_side_grasp_ids(self, actor, arm_tag, limit=GRASP_CANDIDATE_LIMIT):
            ...
            if not ids:
                fallback = self._pick_side_grasp_id_reach(actor, arm_tag)
                ids = [] if fallback is None else [fallback]
            if os.environ.get("ROBOTWIN_LOG_MOVE", "") == "1":
                print(f"[grasp_id] arm={arm_tag} candidates={ids}")
            return ids

        def _pick_side_grasp_id_reach(self, actor, arm_tag):
            ranked = self._rank_side_grasp_ids(actor, arm_tag, limit=1)
            return ranked[0] if ranked else None

        def _geometric_grasp_pose_reach(self, actor, cp_id, pre_dis=0.0):
            """Grasp pose for a contact point, computed geometrically (same math as
            get_grasp_pose but WITHOUT choose_best_pose's planning gate). Used only to
            orient/place the side waypoint, so a box-blocked direct plan can't veto it."""
            cm = actor.get_contact_point(cp_id, "matrix")
        ...
        def _candidate_specs(self, arm_tag, cp_ids):
            for cp_id in cp_ids:
                grasp_pre_pose = self._geometric_grasp_pose_reach(self.target_obj, cp_id, pre_dis=0.1)
                grasp_pose = self._geometric_grasp_pose_reach(self.target_obj, cp_id, pre_dis=0.0)
                ...
                for gap in gaps:
                    x_side = self._box_side_x_reach(arm_tag, gap=gap)
        ...
        def _plan_candidate(self, arm_tag, candidate, grasp_side_cache):
            ...
            x_side = self._box_side_x_reach(arm_tag, gap=candidate["gap"])
            placement_subgoals = self._backward_subgoal_poses_reach(
        ...
        def _select_pick_place_candidate(self, arm_tag):
            ...
            if not cp_ids:
                return {
                    ...
                    "placement_subgoals": self._backward_subgoal_poses_reach(
                        arm_tag,
                        x_side=self._box_side_x_reach(arm_tag),
