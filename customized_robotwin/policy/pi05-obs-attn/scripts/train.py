...
def train_step(
    ...
    def loss_fn(
        ...
        # Obstacle-attention models return (chunked_mse, aux_kl_dict); others a plain array.
        if isinstance(out, tuple):
            chunked_loss, aux_kl = out
        else:
            chunked_loss, aux_kl = out, {}
        mse = jnp.mean(chunked_loss)
        if aux_kl:
            kl_total = jnp.sum(jnp.stack(list(aux_kl.values())))
        else:
            kl_total = jnp.zeros((), dtype=mse.dtype)
        total = mse + kl_total
        aux = {"mse_loss": mse, "kl_loss": kl_total, **aux_kl}
        return total, aux
