...
class Attention(nn.Module):
    ...
    # For obstacle-guided attention: number of trailing query tokens (action tokens)
    # and leading key tokens (base_0_rgb visual tokens) to read attention from.
    num_action_tokens: int = 0
    num_image_tokens: int = 0

    @nn.compact
    def __call__(self, xs, positions, attn_mask, kv_cache, return_attn: bool = False):  # noqa: FBT002
        ...
        # Obstacle-guided attention read-out: action-query -> base_0_rgb-key attention.
        # probs is [B, K, G, T, S]; slice the last `num_action_tokens` query rows and the
        # first `num_image_tokens` key cols, renormalize over visual tokens, then mean over
        # heads (K, G) and action queries -> [B, num_image_tokens]. Computed in fp32.
        attn_readout = None
        if return_attn:
            a = probs[..., -self.num_action_tokens :, : self.num_image_tokens].astype(jnp.float32)
            a = a / jnp.clip(jnp.sum(a, axis=-1, keepdims=True), a_min=1e-8, a_max=None)
            attn_readout = jnp.mean(a, axis=(1, 2, 3))  # [B, num_image_tokens]
...
class Block(nn.Module):
    ...
    # For obstacle-guided attention read-out (see Attention).
    num_action_tokens: int = 0
    num_image_tokens: int = 0

    @nn.compact
    def __call__(self, xs, kv_cache, positions, attn_mask, adarms_cond, deterministic=True, return_attn=False):  # noqa: FBT002
        ...
        post_attn, kv_cache, attn_readout = attn(pre_attn, positions, attn_mask, kv_cache, return_attn)
        ...
        # Second return is stacked over depth by nn.scan. `attn_readout` is None unless
        # obstacle-attention read-out was requested (then it stacks to [depth, B, N]).
        return xs, (kv_cache, attn_readout)
...
class Module(nn.Module):
    ...
    adarms: bool = False
    # For obstacle-guided attention read-out (see Attention/Block).
    num_action_tokens: int = 0
    num_image_tokens: int = 0
    ...
    def setup(self):
        ...
        self.layers = nn.scan(
            ...
            in_axes=(
                ...
                nn.broadcast,
                nn.broadcast,
            ),  # 0=kv_cache, 1=positions, 2=mask, 3=adarms_cond, 4=deterministic, 5=return_attn
    ...
    def __call__(
        ...
        *,
        ...
        embedded, (kv_cache, attn_readout) = self.layers(
            embedded, kv_cache, positions, mask, adarms_cond, deterministic, return_attn
        )
